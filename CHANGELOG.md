...
- **2026-02-21**: Redesigned the primary application navigation by replacing the standard `BottomNavigationBar` with a custom floating "Dynamic Island" style `DynamicNavBar`. This sits at the bottom center of the screen embedded via a `Stack` and features animated icon styles and a sleek drop shadow. Added 100px bottom offset padding to all scroll views to ensure the floating island does not clip content.
